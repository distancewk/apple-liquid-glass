...
| 案例 | 验证重点 | 直接打开 |
| --- | --- | --- |
| 音乐播放 | 真实媒体内容与底部播放控制 | [运行](examples/04-music-player.html) |
| 创意工作台 | 中性画布与顶部共享工具栏 | [运行](examples/05-workbench.html) |
...
这是所有页面都必须遵守的基础前端规则，不是液态玻璃的可选优化：

| 规则 | 正确做法 | 要避免的问题 |
| --- | --- | --- |
| 先定容器 | 先确定页面容器、列宽、间距和最大阅读宽度 | 先做玻璃，再用玻璃宽度反推布局 |
| 控件归属内容区 | 工具栏、输入器、操作条与它控制的内容列同边界 | 以视口为中心，跨过侧栏或兄弟列 |
| 宽度来自所属区域 | 内容层填满自己的玻璃 wrapper；工具栏宽度服从内容 | 玻璃很宽但文字只占一半，留下无意义空材质 |
| 固定定位有上下文 | 必须固定时，使用相同容器公式计算每个断点 | `position: fixed; left: 50%` 导致桌面多列错位 |
| 响应式先于装饰 | 检查溢出、换行、44px 触控目标和窄屏折叠 | 用阴影、模糊或圆角掩盖错误坐标 |

以聊天页面为例，底部输入器属于右侧会话列，所以桌面端必须与会话列的 `x`、宽度和右边界一致；只有在移动端列折叠后，才切换为页面安全边距布局。这个约束已经写入 [SKILL.md](SKILL.md) 和 [验收清单](references/verification.md)，案例见 [06-chat.html](examples/06-chat.html)。
...
三个案例共用同一份严格 Apple 基线：中性界面、系统字体、一个应用 tint 与功能性玻璃。它们只在任务结构和控件拓扑上不同。

### 01 · 音乐播放

专辑封面和演出照片是唯一的表达性色彩；播放队列与界面保持中性，底部播放控制只承担即时操作。
...
中性画布、关系节点与资料列表承载内容；顶部是一个连续的共享玻璃工具栏，蓝色仅表示当前选择与主操作。
